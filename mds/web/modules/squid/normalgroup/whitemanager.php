...
$n = new ListInfos($arrB,"List of words and domains allowed");
$n->setName(_T("List"));

//Create Title and show
$t = new TitleElement(_T("Internet Whitelist Management"), 2);
$t->display();
...
$f->add(new TrFormElement("<strong>" . _T("Keyword or domain to allow access:") . "</strong>" , new InputTpl("whitelistName" )), array("value" => ""));
...
$n->addActionItem(new ActionPopupItem(_("Delete"),"deletew","delete","whitelist") );
$n->setCssClass("squidWhitelist");
$n->display();
...
if (isset($_POST["btnAdd"])) {
	...
	if ((preg_match("/^[\?,\*,\#,\&,\(,\),]/", $whitelistName))) {
		$n1 = new NotifyWidget();
		$n1->add(sprintf(_T("Invalid words %s insert names without special characters"), $whitelistName));
		header("Location: " . urlStrRedirect("squid/normalgroup/whitemanager"));
	} else  {
		$arrB = get_blacklist();
		addElementInWhiteList($whitelistName, $arrB);
		if (!isXMLRPCError()) {
			header("Location: " . urlStrRedirect("squid/normalgroup/whitemanager"));
		}
	}
